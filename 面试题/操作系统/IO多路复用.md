...
## IO多路复用
  ...
  - **也就是**：IO多路复用是一种实现并发的方式，主要处理文件的
      - 它可以监听文件操作符是否准备好，准备好了就通知进程
      - 可以避免程序在等待一个文件描述符的IO操作时，浪费时间和资源。
...
## 文件描述符准备好IO操作
    ...
    - 指文件描述符关联的IO对象可以进行读或写操作，而不会阻塞程序的执行。

    - 例如，一个文件描述符关联的是一个套接字，
        - 当套接字收到数据时，文件描述符就准备好了读操作；
        - 当套接字可以发送数据时，文件描述符就准备好了写操作。
      - 如果一个文件描述符关联的是一个普通文件，那么文件描述符通常总是准备好了读和写操作。
