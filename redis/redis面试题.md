...
## Redis优缺点
  ...
  - **优势**
    ...
    - 支持持久化.		支持RDB 和 AOF两种持久化机制,持久化有效避免数据丢失问题
...
## Redis为什么快?
  ...
  - **基于内存**：Redis使用内存存储,没有磁盘io开销.读写速度快
...
## Redis的线程模型
  ...
  - 基于Reactor模式开发了网络事件处理器,这个处理器被称为文件事件处理器.
...
## Memcached和Redis的区别
  ...
  - value值大小不同,Redis 最大为 512MB,Memcached只有1m
...
## 为什么要用 Redis 而不用 map/guava 做缓存?
  ...
  - 用 Redis 作为分布式缓存,多实例的情况下,各个实例共用一份缓存,有一致性
...
## SortedSet 和 List异同点?
  ...
  - **不同**
    - List 基于链表实现,获取两端元素速度快,访问中间元素速度慢
    - SortedSet基于散列表和跳跃表实现,访问中间元素时间复杂度是O(log N)
    - List 不能简单调整某个元素位置,SortedSet可以
    - SortedSet更耗内存
...
## 持久化机制
  ...
  - 支持两种持久化,
    - **RDB方式**：依据指定规则定时将数据写入磁盘
      - Redis默认的持久化方案,开启后会将内存数据写入磁盘,在指定目录中生成dump.rdb文件
      - Redis重启会加载该文件恢复数据
        - bgsave是主流的触发 RDB持久化的方式,
          - 执行过程如下:
            - BGSAVE命令
...
## Redis Cluster
  ...
  - **原理**
    ...
    - 同一分片多节点间数据不保持一致
    - 读取数据时,客户端操作的key分配在该节点上时,Redis 会返回转向指令,指向正确节点
    - 扩容时要把旧节点数据迁移部分到新节点
...
## 为什么 Redis 集群的最大槽数是 16384 个？
  ...
  - Redis Cluster 采用数据分片，定义了16384个槽，集群中每个Redis负责维护部分槽及槽所映射的键值数据。
...
- **为什么快**：内存 + IO 多路复用 + 高效数据结构；命令执行仍单线程
- **持久化**：RDB 快照 vs AOF 日志；生产常两者结合，重启优先 AOF
- **集群**：主从复制、哨兵故障转移、Cluster 16384 槽分片
- **缓存问题**：穿透（布隆/空值）、击穿（互斥锁/逻辑过期）、雪崩（随机 TTL）
- **分布式锁**：`SET key value NX EX` + 唯一 value；释放用 Lua `del KEYS[1]`
- **线程模型**：文件事件处理器 + Reactor，6.0 多线程仅用于网络 IO
