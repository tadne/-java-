...
- **synchronized优化**：jvm中,每个对象都有对象头(包含了class指针和Mark Word字段)
- MarkWord平时存对象的hash码,分代年龄.
- 加锁时,这个信息就根据情况替换为标记位,线程锁记录指针,重量级指针,线程id等
...
- 没有锁竞争时不需要使用操作系统的同步工具，也不会线程阻塞，效率较高。

- **锁重入**：指线程已经获取到锁时，可以再次获取该对象的锁，而不会导致死锁或异常。
- 锁重入可以保证线程在执行同步代码时的一致性和安全性。

- 如果一个对象虽然有多线程访问,但是多线程访问时间是错开的,那么可以使用轻量级锁来优化

- 在锁竞争较少的时候,可以正常加锁成功,效率高.
...
- 当多个线程同时访问同步代码块时，
- 首先会访问Owner字段，然后通过CAS的方式尝试将Monitor中的owner字段设置为当前线程，
- 如果修改成功,若发现之前的owner的值就是指向当前线程的，recursions也加1
- 如果CAS尝试获取锁失败,就将count加1，则进入到EntryList中。
- 当获取锁的线程调用wait()方法时，会将owner设置为null，同时recursions减1，
- 线程加入到WaitSet中，等待被唤醒。
- 当线程执行完同步代码块时，会释放锁，count减1，recursions减1。
- 当recursions的值为0时，说明线程已经释放了锁。

- count字段用来记录重试次数
- recursions字段用来记录锁重入次数
...
- **锁消除**：jvm用逃逸分析,当某个加锁对象是方法局部变量时,不会被其他线程访问
- 这时会被即使编译器忽略同步操作
